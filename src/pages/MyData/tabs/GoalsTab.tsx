...
export const GoalsTab = () => {
  ...
  return (
    ...
    >
      {/* Header Section */}
      <div className="bg-gradient-to-br from-color8 to-color8/80 rounded-2xl p-8 text-color1 shadow-xl">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold">Financial Goals</h2>
            <p className="mt-2 text-color2">Track and manage your financial aspirations</p>
          </div>
          <motion.button 
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={handleAddGoal}
            className="inline-flex items-center px-6 py-3 bg-color1 text-color8 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 font-medium"
          >
        ...
        <div className="grid grid-cols-3 gap-6 mt-8">
          <div className="bg-color1/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-color2">Total Goals</p>
            <p className="text-3xl font-bold mt-1">{goals.length}</p>
          </div>
          <div className="bg-color1/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-color2">Active Goals</p>
            <p className="text-3xl font-bold mt-1">{goals.filter(g => parseFloat(calculateProgress(g.current, g.target)) < 100).length}</p>
          </div>
          <div className="bg-color1/10 backdrop-blur-sm rounded-xl p-4">
            <p className="text-color2">Completed Goals</p>
            <p className="text-3xl font-bold mt-1">{goals.filter(g => parseFloat(calculateProgress(g.current, g.target)) >= 100).length}</p>
          </div>
        </div>
      </div>
